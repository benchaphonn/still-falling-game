document.addEventListener("DOMContentLoaded", () => {
  ...
  function setPlayerDisplay(div, name, character) {
    div.innerHTML = "";
    ...
    characterDiv.className = "player-character";
  ...
  function renderPlayers(players = []) {
    ...
    if (p1?.character) {
      ...
      setPlayerDisplay(player1Div, p1.name || "Player 1", p1.character);
    }

    if (p2?.character) {
      player2.emoji = p2.character;
      setPlayerDisplay(player2Div, p2.name || "Player 2", p2.character);
    }

    if (!player1Div.textContent && player1.emoji) {
      setPlayerDisplay(player1Div, sessionStorage.getItem("playerName") || "Player 1", player1.emoji);
    }

    if (!player2Div.textContent && player2.emoji) {
      setPlayerDisplay(player2Div, "Player 2", player2.emoji);
    }
  }
  ...
  socket.on("chapter1Ended", (data) => {
    showPopup(Boolean(data.win));
  });
